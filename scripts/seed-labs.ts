...
const LABS = [
  ...
  },
  {
    ...
    claimed_location: 'Buffalo Grove, IL (per the lab\'s own site meta description). A separate, unofficial Facebook page lists "Deerfield, IL" — an adjacent Chicago suburb; likely an imprecise or auto-filled Facebook location rather than a deliberate mismatch, not resolved with certainty.',
    registered_address: null,
    address_mismatch: false,
    founded_claim: null,
    ...
    portal_verified: false,
    what_it_is: 'Kovera Labs is a research-compound testing lab offering HPLC purity/net-content testing and LC-MS identity confirmation, with a public per-COA verification portal (each COA gets a KVR-2026-XXXXXX code resolved on Kovera\'s own server, not the vendor\'s). Domain registered 2026-01-02 (Tucows, WHOIS-privacy-protected), hosted on AWS, email on Google Workspace — a normal small-business technical setup, not a burner/throwaway one. Roughly 7 months old as of this review, one of the newest labs in this registry.',
    accreditation_summary: 'No accreditation claim of any kind was found on the lab\'s own site or in any secondary source — no ISO/IEC 17025, no A2LA listing, no CLIA registration. A direct A2LA registry search could not be completed via automated tools this session (the search requires an interactive form), so absence is inferred from a total lack of any accreditation claim, not a confirmed negative registry search.',
    what_we_verified: 'An independent third-party analysis (vialaudit.com, not our own data pull) of Kovera\'s public verifier examined 1,816 published COAs across 146 vendor-clients as of May 2026 — median purity 99.907%, with 15 explicit purity failures and 7 identity-mismatch records published openly rather than suppressed, a real trust signal. In our own lab_tests data, four tracked vendors carry Kovera-attributed results: Ascension Peptides (50 records), Peptide Partners (15), Glacier Aminos (2), Ion Peptide (1). We independently ran WHOIS/DNS checks on koveralabs.com vs. instantpeptides.com (the vendor a community post alleged shares "IP registration" with Kovera) to test that specific claim directly: different registrars (Tucows vs. GoDaddy), different nameservers (NS1.com vs. Cloudflare), different hosting IPs (AWS vs. a separate host), different mail providers (Google Workspace vs. Microsoft 365) — no technical overlap found on any axis checked. We also read the primary peptidecritic.com community forum thread in full (not a secondhand summary): the "IP overlap" claim originates from a single anonymous poster citing an unspecified "quick google search," is disputed within the same thread by multiple other experienced posters as "a pretty crazy accusation with very little proof," and the thread itself documents a real counter-example (a Glacier Aminos product that failed at Kovera and passed at a different lab) — evidence against a "rubber-stamp for friendly vendors" theory, not for it.',
    what_we_could_not_verify: 'No legal entity name, ownership, or corporate registration confirmed (Illinois Secretary of State and OpenCorporates searches could not be completed via automated tools this session — both are behind interactive forms/CAPTCHAs). Could not personally complete a live verification-portal lookup (no COA code on hand). A LinkedIn profile associated with the lab could not be independently corroborated (LinkedIn blocks automated access). A separate, more serious claim — a MESO-Rx forum poster describing a specific sample that tested ~27% purity but received a passing Kovera result, with an accompanying blacklisting allegation — was surfaced on a first automated page fetch, but the source site actively blocks automated access (confirmed via multiple follow-up attempts, including a Googlebot user-agent, all blocked) and we could not reproduce or re-verify the exact wording on a second pass. The claim is specific enough (an exact purity figure and vial weight) that it plausibly reflects real forum content rather than a fabrication, but it should be treated as unconfirmed and unreproduced, not as a verified finding.',
    caveats: 'Operator/ownership transparency remains a genuine, unresolved gap — no team, credentials, or legal entity are disclosed anywhere, which is consistent with several other small labs in this registry (Horizon Analytical, Chromate at launch) but still a real limitation for a lab whose entire value proposition is independence. The specific vendor-ownership-overlap claim, when checked directly, did not hold up technically. The specific false-pass/blacklisting claim could not be re-verified due to source-site access blocking, not because it was found to be false — treat it as an open question, not a resolved one either way.',
    bottom_line: 'Real and high-volume, not a fabricated identity — the published-failure-rate data and a directly-checked negative on the "IP overlap" claim are both genuine positive signals. Operator anonymity and one specific, unconfirmed-but-unreproducible fraud allegation keep this below the bar of the other four non-accredited labs in this registry, all of which have no comparable open allegation. Re-review after Kovera has a longer operating history, and prioritize re-attempting the MESO-Rx thread verification when the source site is reachable again.',
