...
- **Blood test vendor migrations** — 9 of 14 blood-test vendors still on the flat coverage model; Goodlabs, Marek Diagnostics, Vitals Vault, Jinfiniti (AgingSOS), and SiPhox Health (all 2026-07-13) are the reference implementations for migrating a vendor to the products model (Cart Builder + full catalog). Superpower was evaluated and rejected as a migration candidate: it's a single-tier subscription ($199/yr flat for its whole 359-marker catalog), so there's no bundle/cart choice to build a set-cover algorithm around — the flat model is actually the right long-term fit for it. Checked tier counts for all remaining vendors: most (Function Health, Hundred Health, and the single-tier shape generally) share Superpower's problem. Billing cadence (subscription vs one-time) doesn't actually matter for this model — only whether there are 2+ genuinely different purchasable bundles at different price points. Next-best remaining candidate: **Mito Health** (3 tiers, $90/$192/$1455, unverified whether additive — check before assuming it migrates as cleanly as Vitals Vault/Jinfiniti did).
