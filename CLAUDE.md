...
- **Vendor-category scope question**: `healthgevity`'s real catalog is longevity supplements and skincare (FACEgevity™, NMN 1000+, Telomere Prime+), not peptides — 4 matched, 27 unmatched, and none of the 27 are peptides under any reasonable classification. `swiss-chems`, `limitless-biotech`, and `sports-technology-labs` are predominantly nootropics/SARMs vendors (Piracetam, Noopept, RAD-140, Cardarine, Ostarine, etc. — real compounds, deliberately added to `KNOWN_NON_PEPTIDES`, not missing peptide keywords) with only a handful of genuine peptides each. Whether these still belong on a "peptide vendor" review site is a listing decision, not a scraper bug.
