...
async function main() {
  ...
  try {
    await page.waitForSelector("#zc-manage", { timeout: 3000 });
    await page.evaluate(() => (document.querySelector("#zc-manage") as HTMLElement)?.click());
    await new Promise(r => setTimeout(r, 1000));
  } catch {}

  await page.type('input[name="username"]', "info@watchtowerpeptides.com", { delay: 40 });
  await page.type('input[name="password"]', "jikHip-6dewje-xytgih", { delay: 40 });
  await Promise.all([
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      docLinks: [...links, ...imgs].filter((h: string) => /\.(pdf|png|jpg|jpeg)/i.test(h) || h.includes("janoshik")),
    };
  });

  console.log("\nCOA page URL:", result.url);
  console.log("Body preview:", result.bodyText.slice(0, 400));
  console.log("\nDoc links:", result.docLinks.slice(0, 25).join("\n"));
