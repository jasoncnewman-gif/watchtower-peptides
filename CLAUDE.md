...
- **`/blood-tests` listing page entry pricing (fixed 2026-07-14):** the card grid's displayed price now prefers the cheapest `vendor_test_products` row with `product_type='panel'`, falling back to `vendor_tiers.is_entry_tier` only for vendors with no products yet (currently just InsideTracker). Previously it read `vendor_tiers.is_entry_tier` directly, which drifted stale for several vendors during the 2026-07 products-model migration pass (SiPhox showed $99 instead of $124, Everlywell showed a $39 non-product "membership" instead of a real $249 test) — always re-check this if a vendor's price changes again, since `vendor_tiers` isn't touched by product migrations and will silently go stale. Relatedly, `lab_vendors.true_annual_cost_cents` must stay ≥ the displayed entry price or the card shows a nonsensical "annual cost" lower than the one-time price (this happened for InsideTracker, Mito Health, and Jinfiniti — all fixed by hand 2026-07-14).
