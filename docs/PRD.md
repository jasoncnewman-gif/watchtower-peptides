...
- **Blood test vendor migrations** — 8 of 14 blood-test vendors still on the flat coverage model; Goodlabs, Marek Diagnostics, Vitals Vault, Jinfiniti (AgingSOS), SiPhox Health, and Mito Health (all 2026-07-13) are the reference implementations for migrating a vendor to the products model (Cart Builder + full catalog). Superpower was evaluated and rejected as a migration candidate: it's a single-tier subscription ($199/yr flat for its whole 359-marker catalog), so there's no bundle/cart choice to build a set-cover algorithm around — the flat model is actually the right long-term fit for it. Function Health and Hundred Health share Superpower's single-tier problem (also rejected as-is). Billing cadence (subscription vs one-time) doesn't actually matter for this model — only whether there are 2+ genuinely different purchasable bundles at different price points. Remaining candidates worth checking before assuming rejection: **InsideTracker** (2 tiers: Membership $149/Ultimate $340 — verify these are genuinely different marker sets, not just an upsell), **Everlywell** (2 tiers: $39 general/$399 360 — same check), **Lifeforce** (2 tiers but named "Peptide Membership (funnel-specific)" vs "General Membership (main funnel)" — likely the same product marketed two ways, not real bundle diversity, but unverified), **Rythm Health** and **OneTwenty** look like the weakest remaining candidates (Rythm has one tier priced $0, likely a data artifact; OneTwenty's two tiers differ only by NY/NJ surcharge, not content).
