...
- **Blood test vendor migrations** — 11 of 14 blood-test vendors still on the flat coverage model; Goodlabs, Marek Diagnostics, and Vitals Vault (all 2026-07-13) are the reference implementations for migrating a vendor to the products model (Cart Builder + full catalog). Superpower was evaluated and rejected as a migration candidate: it's a single-tier subscription ($199/yr flat for its whole 359-marker catalog), so there's no bundle/cart choice to build a set-cover algorithm around — the flat model is actually the right long-term fit for it. Prefer panel/à-la-carte vendors (like Marek and Vitals Vault, both `business_model: panel`) for future migrations; membership-model vendors (Mito Health, Function Health, InsideTracker, etc.) likely have the same non-fit as Superpower and should be checked for tier structure before assuming they're good candidates. Next-best remaining `panel`/`ala-carte` candidate: Jinfiniti AgingSOS.
