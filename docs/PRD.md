...
- **Cart Builder (per-vendor, products-model vendors only)** — pick peptides, get the actual cheapest combination of that vendor's real panels + à la carte tests to cover it, with a toggle between "cheapest for exactly what I need" and "best overall value" (credits a panel's non-target markers too, since someone getting bloodwork anyway benefits from broader data). Goodlabs and Marek Diagnostics are migrated to this model so far — see `CLAUDE.md`'s "Blood Test Vendor Data Model" for the schema and how to migrate another vendor onto it. The other 12 blood-test vendors still show a flatter coverage table until migrated the same way.
...
- **Blood test vendor migrations** — 12 of 14 blood-test vendors still on the flat coverage model; Goodlabs and Marek Diagnostics (2026-07-13) are the reference implementations for migrating a vendor to the products model (Cart Builder + full catalog). Superpower was evaluated and rejected as a migration candidate: it's a single-tier subscription ($199/yr flat for its whole 359-marker catalog), so there's no bundle/cart choice to build a set-cover algorithm around — the flat model is actually the right long-term fit for it. Prefer panel/à-la-carte vendors (like Marek) for future migrations; membership-model vendors (Mito Health, Function Health, InsideTracker, etc.) likely have the same non-fit as Superpower and should be checked for tier structure before assuming they're good candidates.
