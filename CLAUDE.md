...
- **`peptidology.com` no longer resolves (DNS failure)** — also dead, but the vendor's page still shows 38 stale products from its last successful scrape, since a 0-result scrape never overwrites existing `vendor_peptides` rows. This is a real trust/accuracy gap: a delisted-in-fact vendor page currently looks fully live.
